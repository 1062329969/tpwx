...
use app\admin\model\HairCuringImgsModel;
use app\admin\model\HairCuringModel;

class HairCuring extends Admin
{
	protected $modelName='';
	...
	{
		parent::_initialize();
		$this->modelName=new HairCuringModel;
	}
		...
		if($request->method()=='POST')
		{
			...
			if($this->modelName->allowField(true)->save($data))
			{
				...
				if(count($imgs)>0)
				{
					foreach($imgs as $k=>$v)
					{
						$HairCuringModel=new HairCuringImgsModel;
						$HairCuringModel->save(['aid'=>$this->modelName->id,'pic'=>$v]);
					}
				}
	...
	public function alter()
	{
		...
		if($request->method()=='POST')
		{
			...
			if($this->modelName->allowField(true)->save($data,['id'=>$data['id']]))
			{
				...
				if(count($imgs)>0)
				{
					...
					{
						$HairCuringModel=new HairCuringImgsModel();
						$HairCuringModel->save(['aid'=>$data['id'],'pic'=>$v]);
					}
				}
		...
		else
		{
			...
			//读取该头发养护的所有图片
			$newsImgs=HairCuringImgsModel::getAll($id);
	...
	public function delImg()
	{
		...
		HairCuringImgsModel::where(['id'=>$id,'pic'=>$imgurl])->delete();
		return ['code'=>1];
